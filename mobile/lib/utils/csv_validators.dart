...
class CsvValidators {
  ...
  static const List<String> _residentRequiredHeaders = [
    'name',
    'email',
    'phone',
    'flatno'
  ...
  static String normalizeHeader(String header) {
    return header
        .trim()
        .toLowerCase()
        .replaceAll(RegExp(r'^\uFEFF'), ''); // Remove BOM
  }
  ...
  static String? normalizePhone(String phone) {
    ...
    if (cleaned.startsWith('+91')) {
      ...
      if (withoutPrefix.length == 10 &&
          RegExp(r'^\d{10}$').hasMatch(withoutPrefix)) {
  ...
  static bool isValidRole(String? role) {
    if (role == null || role.trim().isEmpty) {
      return true; // Optional, default to resident
    }
      ...
      for (int i = 1; i < csvData.length; i++) {
        ...
        if (phone.isEmpty) {
          ...
          if (normalizedPhone == null) {
            errors.add(
                'Invalid phone format (must be 10 digits or +91 followed by 10 digits)');
          } else {
  ...
  static ValidationResult validateResidentsCsv(String csvText) {
    ...
    final seenPhones = <String>{}; // Track duplicate phones
    final seenFlatNos =
        <String>{}; // Track duplicate flat numbers (for warnings)
      ...
      for (int i = 1; i < csvData.length; i++) {
        ...
        if (flatNo.isEmpty) {
          ...
          if (seenFlatNos.contains(normalizedFlatNo.toUpperCase())) {
            warnings.add(
                'Row $rowNumber: Duplicate flat number "$normalizedFlatNo" (may be intentional for multiple residents)');
          } else {
